...
from models import Numero, Gasto
...
def stats(db: Session = Depends(get_db)):
    vendidos = db.query(func.count(Numero.numero)).filter(Numero.vendido == True).scalar()
    recaudado = db.query(func.sum(Numero.monto)).filter(Numero.vendido == True).scalar() or 0.0
    total_gastos = db.query(func.sum(Gasto.monto)).scalar() or 0.0
    return {
        "vendidos": vendidos,
        "disponibles": 1000 - vendidos,
        "recaudado": recaudado,
        "total_gastos": total_gastos,
        "saldo_neto": recaudado - total_gastos,
    }
...
def crear_gasto(
    ...
    g = Gasto(descripcion=descripcion.strip(), monto=monto)
    db.add(g)
    db.commit()
    db.refresh(g)
